...
def featurize_page_soup(soup, link):
    ...
    OUTPUT: page_data - dict of additional info from page_soup
    '''
...
if __name__ == '__main__':
    ...
    cursor = db.prnewswire.find({ '$query': {'source': 'personnel-announcements-list',
                                             'pulled': {'$exists': False}},
                                  '$snapshot': True})
    count = 1
    tot = db.prnewswire.find().count()
    for doc in cursor:
        print doc['_id']
        ...
        db.prnewswire.update_one({'_id': doc['_id']}, {'$set': {'pulled': 'yes'}})
        count += 1
        if count == 10:
            break
    cli.close()
